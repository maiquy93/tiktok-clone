...
import { loginStateSelector, userdataSelector } from "redux/selector";
...
function Header() {
  ...
  const handleMenuChange = optionChoose => {
    ...
    switch (optionChoose.title) {
      case "language": //handle change language
        break;
      case "Log out":
        localStorage.setItem("isLogin", JSON.stringify(false));
        localStorage.setItem("userdata", JSON.stringify({}));
        dispatch(loginStateCreator(false));
        ...
        <div className={cx("action")}>
          {value ? (
            <div className={cx("login-bar")}>
              ...
              <Tippy content="Message" delay={100}>
                <button className={cx("message-btn")}>
                  <span className={cx("msg-noctification", "noc-show")}>
                    {3}
                  </span>
          ...
          <Option
            items={value ? USER_MENU : GUESS_MENU}
